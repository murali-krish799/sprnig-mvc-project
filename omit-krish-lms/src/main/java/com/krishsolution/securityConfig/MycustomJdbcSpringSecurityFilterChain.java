...
@Order(1)
public class MycustomJdbcSpringSecurityFilterChain {

	public MycustomJdbcSpringSecurityFilterChain() {

		System.out.println("MycustomJdbcSpringSecurityFilterChain class called...");
	}
	...
	public UserDetailsService userDetailsService() {
		...
		jdbcUserDetailsService.setUsersByUsernameQuery(
				"select user_name,user_password,user_isActive "
						+ "from registration "
						+ "where user_name = ?"				
				);
	...
	public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
		http.authorizeHttpRequests(auth -> auth.requestMatchers("/test", "/test-demo", "/WEB-INF/view/**","/registration","/registration-process").permitAll()
				.requestMatchers("/show-instructor-page").hasAnyAuthority("user", "admin")
